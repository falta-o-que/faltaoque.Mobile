...
- The root agent is the orchestrator and should use `gpt-6-astra` with `low` reasoning by default.
...
- Recommend that the user raise the root Astra to `medium` when ambiguity, integration complexity, security judgment, or difficult debugging warrants it, explaining the concrete reason. Do not claim to change the root model or effort yourself.
- Delegate directly to the suitable worker; do not require an Astra-to-Sol-to-Luna chain. Explain the delegation split at the start of substantial implementation work.
